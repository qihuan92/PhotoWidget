...
fun Uri.saveGifFramesToDir(
    ...
    val gifDrawable = Glide.with(context.applicationContext)
        .asGif()
        .load(this)
        .submit()
        .get()
    ...
    try {
        ...
        for (index in 0..standardGifDecoder.frameCount) {
            ...
            val roundedFrame = frame?.withRoundedCorner(context, radius, radiusUnit)
...
fun Bitmap.withRoundedCorner(context: Context, radiusAngle: Float, radiusUnit: RadiusUnit): Bitmap {
...
fun Bitmap.withRoundedCorner(context: Context, radius: Int): Bitmap {
    var builder = Glide.with(context.applicationContext)
        .asBitmap()
        .load(this)
